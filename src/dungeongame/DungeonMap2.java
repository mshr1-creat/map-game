...
public class DungeonMap2 {
	public static int Map2(int[][] map2) {
		// **** 主人公の位置を表す変数 **** //
		int myX = 1;	// 横位置			// ← 追加
		int myY = 1;	// 縦位置			// ← 追加
		...
		do {
			...
			key = new java.util.Scanner(System.in).nextLine();	// キーボードの入力を受け付ける
				switch(key) {
					case "m" -> {
						// マップのコピーを作成
	                    int[][] mapDisplay = new int[map2.length][map2[0].length];
	                    for (int i = 0; i < map2.length; i++) {
	                        for (int j = 0; j < map2[i].length; j++) {
	                            mapDisplay[i][j] = map2[i][j];
	                        }
	                    }
					...
					case "s" ->{
						...
						if(map2[myX+1][myY]== 0||map2[myX+1][myY]== 2||map2[myX+1][myY]== 4){
							myX++;
							if(map2[myX][myY]==2) {
								System.out.println("おめでとう！ダンジョンをクリアしました！");
							}else if(map2[myX][myY]==4) {
								myX = 1;
								myY = 1;
					...
					case "a" ->{
						...
						if(map2[myX][myY-1]==0||map2[myX][myY-1]==2||map2[myX][myY-1]==4) {
							myY--;
							if(map2[myX][myY]==2) {
								System.out.println("おめでとう！ダンジョンをクリアしました！");
							}else if(map2[myX][myY]==4) {
